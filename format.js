function format(number,displayNumber) {
    if (number == depth || number == dtrm.cost) {
        if (number.gte(1000000) && number.lt(1000000000)) {
            ...
            return (displayNumber.toStringWithDecimalPlaces(4) + "Mm");
        } if (number.gte(1000) && number.lt(1000000)) {
            displayNumber = displayNumber.div(1000);
            return (displayNumber.toStringWithDecimalPlaces(4) + "km");
        } else {
            return (displayNumber.toStringWithDecimalPlaces(4) + "m");
        };
    } else {
        if (number.gte(1000) && number.lt(1000000)) {
            displayNumber = displayNumber.div(1000);
            return (displayNumber.toStringWithDecimalPlaces(4) + "k");
        } if (number.gte(0) && number.lt(1000)) {
            return (displayNumber.toStringWithDecimalPlaces(4));
        };
    };
};
